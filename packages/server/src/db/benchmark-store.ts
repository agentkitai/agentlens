...
export class BenchmarkStore {
   ...
   */
  async create(tenantId: string, input: CreateBenchmarkInput): Promise<BenchmarkWithVariants> {
    ...
    // Build the inserts up front, then run them atomically on either dialect.
    const queries: SQL[] = [
      sql`
   ...
   */
  async getById(tenantId: string, id: string): Promise<BenchmarkWithVariants | null> {
    const row = await dbGet<BenchmarkRow>(this.db, sql`
      SELECT * FROM benchmarks WHERE id = ${id} AND tenant_id = ${tenantId}
    `);
    if (!row) return null;

    const variantRows = await dbAll<VariantRow>(this.db, sql`
  ...
  async list(
  ...
  ): Promise<{ benchmarks: BenchmarkWithVariants[]; total: number }> {
    ...
    const rows = await dbAll<BenchmarkRow>(this.db, sql`
    ...
    const benchmarks = await Promise.all(rows.map(async (row) => {
      const variantRows = await dbAll<VariantRow>(this.db, sql`
        SELECT * FROM benchmark_variants
   ...
   */
  async updateStatus(tenantId: string, id: string, newStatus: BenchmarkStatus): Promise<BenchmarkWithVariants> {
   ...
   */
  async saveResults(tenantId: string, benchmarkId: string, results: BenchmarkResults): Promise<void> {
    const id = randomUUID();
   ...
   */
  async getResults(tenantId: string, benchmarkId: string): Promise<BenchmarkResults | null> {
