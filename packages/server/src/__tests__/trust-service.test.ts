...
describe('TrustService — Story 6.3 (Trust Scoring)', async () => {
  let db: SqliteDb;
  ...
  async function insertHealthSnapshot(agentId: string, date: string, score: number) {
    await healthStore.save('tenant-1', {
      agentId,
      date,
      overallScore: score,
  ...
  describe('computeRawTrustScore', async () => {
    it('should return default score (50) when no data exists', async () => {
      const score = await trustService.computeRawTrustScore('tenant-1', 'agent-1');
      // 0.6 * 50 + 0.4 * 50 = 50
      expect(score).toBe(50);
    });

    it('should use health scores when available', async () => {
      await insertHealthSnapshot('agent-1', recentDate(0), 80);
      await insertHealthSnapshot('agent-1', recentDate(1), 60);
      const score = await trustService.computeRawTrustScore('tenant-1', 'agent-1');
    ...
    it('should use delegation success rate when available', async () => {
    ...
    it('should combine health and delegation components', async () => {
      await insertHealthSnapshot('agent-1', recentDate(0), 90);
    ...
    it('should handle 0% delegation success rate', async () => {
    ...
    it('should handle perfect health and delegation scores', async () => {
      await insertHealthSnapshot('agent-1', recentDate(0), 100);
      for (let i = 0; i < 10; i++) insertDelegationLog('agent-1', 'completed');
      const score = await trustService.computeRawTrustScore('tenant-1', 'agent-1');
      expect(score).toBe(100);
    });

    it('should only count terminal delegation statuses', async () => {
      ...
      const stats = await trustService.getDelegationStats('tenant-1', 'agent-1');
      expect(stats.total).toBe(1);
    });

    it('should only count outbound delegations', async () => {
  ...
  describe('computeHealthComponent', async () => {
    it('should return 50 when no health data exists', async () => {
      const component = await trustService.computeHealthComponent('tenant-1', 'agent-1');
      expect(component).toBe(50);
    });

    it('should average health scores over last 30 days', async () => {
      await insertHealthSnapshot('agent-1', recentDate(0), 80);
      await insertHealthSnapshot('agent-1', recentDate(1), 60);
      await insertHealthSnapshot('agent-1', recentDate(2), 70);
      const component = await trustService.computeHealthComponent('tenant-1', 'agent-1');
      expect(component).toBe(70);
    });

    it('should handle single health snapshot', async () => {
      await insertHealthSnapshot('agent-1', recentDate(0), 85);
      const component = await trustService.computeHealthComponent('tenant-1', 'agent-1');
  ...
  describe('computeDelegationComponent', async () => {
    it('should return 50 when no delegations', async () => {
      const component = await trustService.computeDelegationComponent({ total: 0, successful: 0, failed: 0, timedOut: 0 });
      expect(component).toBe(50);
    });

    it('should compute 100% for all successful', async () => {
      const component = await trustService.computeDelegationComponent({ total: 10, successful: 10, failed: 0, timedOut: 0 });
      expect(component).toBe(100);
    });

    it('should compute 0% for all failed', async () => {
      const component = await trustService.computeDelegationComponent({ total: 10, successful: 0, failed: 10, timedOut: 0 });
      expect(component).toBe(0);
    });

    it('should compute mixed success rate', async () => {
      const component = await trustService.computeDelegationComponent({ total: 10, successful: 7, failed: 2, timedOut: 1 });
      expect(component).toBe(70);
    });
  });
  ...
  describe('getDelegationStats', async () => {
    it('should return zero stats when no delegations', async () => {
      const stats = await trustService.getDelegationStats('tenant-1', 'agent-1');
      expect(stats).toEqual({ total: 0, successful: 0, failed: 0, timedOut: 0 });
    });

    it('should count completed delegations', async () => {
    ...
    it('should count timed out delegations', async () => {
    ...
    it('should count error and rejected as failed', async () => {
    ...
    it('should isolate by agent', async () => {
      insertDelegationLog('agent-1', 'completed');
  ...
  describe('getTrustScore', async () => {
    it('should return full trust score object', async () => {
      const score = await trustService.getTrustScore('tenant-1', 'agent-1');
    ...
    it('should mark as provisional with < 10 delegations', async () => {
      for (let i = 0; i < 9; i++) insertDelegationLog('agent-1', 'completed');
      const score = await trustService.getTrustScore('tenant-1', 'agent-1');
      expect(score.provisional).toBe(true);
    });

    it('should mark as non-provisional with >= 10 delegations', async () => {
      for (let i = 0; i < 10; i++) insertDelegationLog('agent-1', 'completed');
      const score = await trustService.getTrustScore('tenant-1', 'agent-1');
      expect(score.provisional).toBe(false);
    });

    it('should include correct delegation counts', async () => {
    ...
    it('should round raw score to 2 decimal places', async () => {
      await insertHealthSnapshot('agent-1', recentDate(0), 73);
  ...
  describe('computePercentile', async () => {
    it('should return 100 when only one agent', async () => {
      capStore.create('tenant-1', 'agent-1', baseCap as any);
      const percentile = await trustService.computePercentile('tenant-1', 'agent-1');
      expect(percentile).toBe(100);
    });

    it('should rank agents correctly', async () => {
      ...
      const p1 = await trustService.computePercentile('tenant-1', 'agent-1');
      const p2 = await trustService.computePercentile('tenant-1', 'agent-2');
      expect(p1).toBeGreaterThan(p2);
    });

    it('should handle agents with equal scores', async () => {
      capStore.create('tenant-1', 'agent-1', baseCap as any);
      capStore.create('tenant-1', 'agent-2', baseCap as any);
    ...
    it('should rank across 3 agents', async () => {
      capStore.create('tenant-1', 'agent-1', baseCap as any);
      capStore.create('tenant-1', 'agent-2', baseCap as any);
      capStore.create('tenant-1', 'agent-3', baseCap as any);

      await insertHealthSnapshot('agent-1', recentDate(0), 90);
      await insertHealthSnapshot('agent-2', recentDate(0), 50);
      await insertHealthSnapshot('agent-3', recentDate(0), 20);

      const p1 = await trustService.computePercentile('tenant-1', 'agent-1');
      const p3 = await trustService.computePercentile('tenant-1', 'agent-3');
      expect(p1).toBe(100);
      expect(p3).toBe(0);
    });
  });
  ...
  describe('updateAfterDelegation', async () => {
    it('should update capability_registry quality metrics', async () => {
      const cap = capStore.create('tenant-1', 'agent-1', baseCap as any);
      for (let i = 0; i < 5; i++) insertDelegationLog('agent-1', 'completed');
      await trustService.updateAfterDelegation('tenant-1', 'agent-1');
    ...
    it('should not throw when agent has no capabilities', async () => {
      await expect(trustService.updateAfterDelegation('tenant-1', 'agent-no-caps')).resolves.toBeUndefined();
    });

    it('should reflect delegation outcomes in trust score', async () => {
      const cap = capStore.create('tenant-1', 'agent-1', baseCap as any);
      for (let i = 0; i < 10; i++) insertDelegationLog('agent-1', 'completed');
      await trustService.updateAfterDelegation('tenant-1', 'agent-1');
