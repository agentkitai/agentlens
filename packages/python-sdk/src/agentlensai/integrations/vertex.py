"""Auto-instrumentation for Google Vertex AI (generative models).
...
@register("vertex")
class VertexInstrumentation(BaseLLMInstrumentation):
    provider_name = "vertex"

    def _get_patch_targets(self) -> list[PatchTarget]:
        return [
            PatchTarget(
                module_path="google.cloud.aiplatform.generative_models",
    ...
    def _extract_call_data(
        ...
        # Extract model from self (first positional arg is the GenerativeModel instance)
        # The base class wrapper passes *args where args[0] is `self` of the patched method
